...
def process_files(yymm):
    ...
    #
    if not prev_yymm:
        Qsa_value, state_fare_dur = {}, {}
        locations = [IN_AIRPORT, OUT_AIRPORT]
        actions = [IN_AIRPORT, OUT_AIRPORT]
        for s1 in DAY_OF_WEEK:
            for s2 in TIME_SLOTS:
                for s3 in locations:
                    state_fare_dur[(s1, s2, s3)] = [0, 0]
                    for a in actions:
                        Qsa_value[(s1, s2, s3, a)] = 0
    else:
        Qsa_value, state_fare_dur = load_picle_file('%s/q-value-fare-dur-%s.pkl' % (for_learning_dir, prev_yymm))
    #
    count = 0
    with open('%s/trip-for-learning-%s.csv' % (for_learning_dir, yymm), 'rb') as r_csvfile:
        ...
        for row in reader:
            ...
            if tm == DInAP_PInAP or tm == DOutAP_PInAP:
                state_fare_dur[(s1_new, s2_new, IN_AIRPORT)][0] += fare
                state_fare_dur[(s1_new, s2_new, IN_AIRPORT)][1] += dur
            else:
                state_fare_dur[(s1_new, s2_new, OUT_AIRPORT)][0] += fare
                state_fare_dur[(s1_new, s2_new, OUT_AIRPORT)][1] += dur
            #
#             if tm == DInAP_PInAP or tm == DOutAP_POutAP:
#                 setup_cost = 0
            if tm == DInAP_PInAP or tm == DInAP_POutAP:
                if state_fare_dur[(s1, s2, IN_AIRPORT)][1] == 0:
                    setup_cost = 0
                else:
                    setup_cost = setup_time * state_fare_dur[(s1, s2, IN_AIRPORT)][0] / state_fare_dur[(s1, s2, IN_AIRPORT)][1]
            elif tm == DOutAP_POutAP or tm == DOutAP_PInAP:
                if state_fare_dur[(s1, s2, OUT_AIRPORT)][1] ==0:
                    setup_cost = 0
                else:
                    setup_cost = setup_time * state_fare_dur[(s1, s2, OUT_AIRPORT)][0] / state_fare_dur[(s1, s2, OUT_AIRPORT)][1]
            else:
                assert False
            #
            ...
            if count % MOD_STAN == 0:
                ...
                save_pickle_file('%s/q-value-fare-dur-%s.pkl' % (for_learning_dir, yymm), [Qsa_value, state_fare_dur])
