...
from supports.location_check import check_terminal_num
    ...
    with open('%s/%s' % (merged_trip_dir, fn), 'rb') as r_csvfile:
        ...
        with open('%s/whole-trip-%s.csv' % (trips_dir, yymm), 'wt') as w_csvfile:
            ...
            for row in reader:
                ...
                c_start_ter = check_terminal_num(eval(row[id_s_long]), eval(row[id_s_lat]))
                c_end_ter = check_terminal_num(eval(row[id_e_long]), eval(row[id_e_lat]))
                if not vehicle_prev_trip_position_time.has_key(vid):
                    ...
                    vehicle_prev_trip_position_time[vid] = (-1, start_time)
                prev_trip_end_ter, prev_trip_time = vehicle_prev_trip_position_time[vid]
                if prev_trip_end_ter != -1 and c_start_ter != -1 :
                    trip_mode = DInAP_PInAP
                elif prev_trip_end_ter != -1 and c_start_ter == -1:
                    trip_mode = DInAP_POutAP
                elif prev_trip_end_ter == -1 and c_start_ter != -1:
                    trip_mode = DOutAP_PInAP
                elif prev_trip_end_ter == -1 and c_start_ter == -1:
                    trip_mode = DOutAP_POutAP
                else:
                    assert False
                new_row = [row[id_tid], vid, row[id_did],
                           start_time, end_time,
                           row[id_dur], row[id_fare],
                           trip_mode, prev_trip_time]
                writer.writerow(new_row)
                #
                vehicle_prev_trip_position_time[vid] = (c_end_ter, end_time)
